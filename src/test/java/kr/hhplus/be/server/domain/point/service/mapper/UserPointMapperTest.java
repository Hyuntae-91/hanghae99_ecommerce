...
class UserPointMapperTest {
    ...
    @Test
    @DisplayName("성공: ApplyCouponDiscountCompletedEvent + PointUseServiceResponse → PointUsedCompletedEvent 변환")
    void toPointUsedCompletedEvent_success() {
        // given
        ApplyCouponDiscountCompletedEvent event = new ApplyCouponDiscountCompletedEvent(
                2L, 20L, 88L, 3000L, List.of(101L, 102L)
        );

        PointUseServiceResponse response = new PointUseServiceResponse(20L, 3000L);

        // when
        PointUsedCompletedEvent result = mapper.toPointUsedCompletedEvent(event, response);

        // then
        assertThat(result.orderId()).isEqualTo(2L);
        assertThat(result.userId()).isEqualTo(20L);
        assertThat(result.usedPoint()).isEqualTo(3000L);
        assertThat(result.productIds()).containsExactly(101L, 102L);
    }
}